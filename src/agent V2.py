...
class Agent:
    ...
    def get_reward(self):
        ...
        """
        reward = 0
        if self.last_action < 1: #reward going forward
            reward += 2
        
        if self.observed_state == 0:
            reward += 3
        
        reward = reward - self.sensors_triggered + self.previous_sensors - self.sensors_close   #penalize trigering sensors and further so when closer to objects
        print(reward)
        return reward
